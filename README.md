...
첫 화면에는 **가짜 데이터를 한 줄도 넣지 않는다.** 이 도구의 결과는 "내
컴퓨터의 경로" 라서, 남의 경로를 예시로 깔면 검색 결과처럼 보여 오히려
해롭다. 대신 세 단계가 각각 무엇을 열어 주는지를 적어 두고, 예시는
`예시로 둘러보기` 로 명시적으로 켠다 (배너가 "당신의 파일이 아니다" 라고
알리고 언제든 끌 수 있다).

IndexedDB 가 막힌 환경(시크릿 창, 사이트 데이터 차단)에서도 화면은 뜬다 —
첫 프레임이 저장소를 기다리지 않는다. 저장된 인덱스가 이전 형식이면 조용히
버리지 않고 "다시 인덱싱해라" 라고 알린다.
...
가장 오래 걸리는 단계라 %만 보여 주면 아무 감이 없다. 그래서 파일 총 개수를
달까지의 거리(384,400 km)로 환산해 🌍 → 🚀 → 🌕 로 보여 준다.
[moon_mission](https://themoka7.github.io/dev-tools/tools/etc/moon_mission.html)
에서 가져온 발상이다.

환산이 일관되므로 **장식이 아니다** — 파일 하나 = `384400 / 전체` km 이고
속도 = `파일초당 × 파일당 km × 3600` 이라, `남은 거리 ÷ 속도` 가 실제 남은
시간과 정확히 같다. 같은 수치의 다른 눈금일 뿐이다. `web/selftest.js` 가
이 등식을 검증한다 — 안 맞으면 그냥 장식이므로.
