...
브라우저 API 는 순회하지 않고 개수를 알려주지 않는다. 그래서 진행률의
분모를 만들 방법이 없다 — 단계를 나눠서 해결했다.

| | 1단계 · 이름 | 2단계 · 크기·시각 |
|---|---|---|
| 하는 일 | 트리 열거 | 파일마다 `getFile()` |
| 비용 | 디렉터리당 1 왕복 | **파일당 1 왕복** |
| 진행률 | 없음 (발견 개수·처리량만) | 진짜 % + 남은 시간 |
| 끝나면 | 검색 가능, 총계 확정 | 크기 필터·중복 검사 가능 |

2단계가 압도적으로 비싸므로 기본은 꺼져 있다. 두 단계 모두 요청을 동시에
여러 개 던진다(`동시 요청`, 기본 32) — 지연 시간에 묶인 작업이라 동시성이
곧 속도다. 이걸 1 로 두면 D: 같은 규모에서 몇 분씩 걸린다.

2단계는 1단계가 만든 트리를 재사용한다. 각 디렉터리 항목이 자식의 연속
범위(`childStart`/`childCount`)를 들고 있어서 이름으로 자식을 O(자식 수)에
찾는다. 핸들을 100 만 개 보관하지 않아도 되고 큰 맵도 필요 없다.
